...
class DataReader(object):
    ...
    def loadDialog(self):
        ...
        for d in self.dialog:
            ...
            for t in range(len(d['dial'])):
                tcount += 1
                turn = d['dial'][t]
                # extract system side sentence feature    #获取系统说的这句话的特征。
                sent = turn['sys']['sent']
                mtar, tar, spos, vpos, venues \
                    = self.extractSeq(sent,type='target')

                # store sentence group#变分推断用的    不太懂
                utt_group.append(self.sentGroup[groupidx])
                groupidx += 1
                
                # changing offer label   用户话中说道的value和以前提到的value不一样的时候
                ...
                if len(mtar)>maxtar: 
                    maxtar = len(mtar) 
                m_targetutt.append(mtar) #系统端说的话加入到了m_targetutt中
    ...
    def extractSeq(self,sent,type='source',normalise=False,index=True):
        ...
        if index:#对没有进行delexicalised的词进行编码
        ...
        allvs = self.infovs+self.reqs #所有的slot value对
        sltpos = [[] for x in allvs]
        valpos = [[] for x in allvs]
        names = []#不太懂为什么会把后面的以value_name开头的slot存起来?????????????????????????????/
        for i in range(len(words)):
            ...
            tok, ID = words[i].split("::")#tok 对应于slot ID对应于value
            words[i] = tok
            # record position        #这一步也不太清楚是做什么的？？？？？？
            mytok,sov = tok[1:-1].lower().split('_')
            ID = ID.replace('-',' ')
            mylist = sltpos if mytok=='slot' else valpos
            for j in range(len(allvs)):#其中i是当前句子中某个词的索引，j是所有的slot value对的索引
                s,v = allvs[j].split('=')
                comp = s if mytok=='slot' else v
                if comp==ID:
                    if mytok=='slot':
                        sltpos[j].append(i)#代表着当前这句话中某个slot出现了多少次
                    else:
                        valpos[j].append(i)#代表着当前这句话中某个value出现了多少次
        ...
        if index:
            ...
            midx = words
        #midx是对已经进行了leximal的词进行的编码，idx是对没有laximal的词进行的编码。
