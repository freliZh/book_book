...
class NNSDS(BaseNNModule):
    ...
    def config_theano(self):
        ...
        # regularization and learning rate学习率
        lr   = T.scalar('lr')
        reg  = T.scalar('reg')
        beta = T.scalar('beta')
        ...
        def dialog_recur(source_t, target_t, source_len_t, target_len_t,
            ...
            if self.trk=='rnn' and self.inf==True:
                #从这个地方可以看出，算法是对所有的 slot value进行遍历，最后生成了一个向量连接了所有的slot value的置信度
                #而且对于每一个slot value都有一个单独的cnn网络与之相对应。
